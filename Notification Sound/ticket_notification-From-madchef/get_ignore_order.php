...
if (isset($_POST['brand']) && isset($_POST['branch'])) {
	$today =date("Y-m-d");
	$sql = "SELECT `id`,`brand`,`branch`,`order_id`,`cart_time` FROM `order` WHERE `cart_time`>='".$today."' AND `order_status`='New' AND `brand`='".$_POST['brand']."' AND `branch`='".$_POST['branch']."' AND `message_status`='0'";

	$data = mysqli_query($con,$sql);
	while ($row = mysqli_fetch_assoc($data)) {
		$now = strtotime(date("Y-m-d H:i:s"));
		$create_at = strtotime($row['cart_time']);
		$expaire_at =strtotime('+15 minutes',$create_at);
		if($expaire_at < $now){
...
function send_message ($ticket_id,$brand,$branch,$order_id){
	...
	// $phone = "+8801639199218";
	$phone_number = "01711084673,01733788309,01305785585,01937428628,01715568511,01710454086";
	// $phone_number = "01639199218,01792788718";
	...
	foreach ($phone_array as $key => $phone) {
		...
		$url = "https://sms.novocom-bd.com/api/v2/SendSMS?ApiKey=eVgQP1P+oqQLcJab6knGSp7Nk8gCcggIu/BJ/6V/1oE=&ClientId=954e64a0-725d-45fc-b24d-14114c825d0e&SenderId=8809638050505&Message=$message&MobileNumbers=$contact&Is_Unicode=true&Is_Flash=true";
...
function update_status($ticket_id){
	$sql = "UPDATE `ticket` SET `message_status`='1' WHERE `id`='".$ticket_id."'";
